...
for i = 1:n1 %Loop for row in the n1*n2 space
    for j = 1:n2 %Loop for each column in the n1*n2 space
         ...
         if (x - cos(radtheta) * P1)^2 + (y - sin(radtheta)*P2)^2 < ceil(r^2) %inside TL circle
            UnitCell(ni, :) = [x y EpsC];
            color='r';
         elseif (x - (P1 + cos(radtheta) * P1))^2 + (y - sin(radtheta)*P2)^2 < r^2 %inside BL circle
            UnitCell(ni, :) = [x y EpsC];
            color='r';
         elseif x^2 + y^2 < r^2 %inside TR circle
            UnitCell(ni, :) = [x y EpsC];
            color='r';
